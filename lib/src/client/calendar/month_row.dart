...
class MonthRow extends CLElement {
  CLElement<TableElement> tableMain, tableGrid;

  CLElement<TableSectionElement> tbodyMain, tbodyGrid;

  CLElement filterCont;
  ...
  MonthRow(List<DateTime> dates, this.calendar) : super(new DivElement()) {
    evCol = new EventCollection();
    fCol = new FilterCollection();
    this.dates = dates.map(EventCalendar.normDate).toList();
    createDom();
    filterCont = new CLElement(new DivElement())..setClass('filter-cont');
    append(filterCont);
  }
  ...
  void setHeight(Dimension dim) {
    super.setHeight(dim);
    final test = new CLElement(new DivElement())
      ..setClass('event-cont')
      ..append(new SpanElement()..text = 'test');
  ...
  void render() {
    ...
    data.forEach((event_row) {
      ...
      event_row.forEach((event) {
        ...
        final cell =
            new CLElement<TableCellElement>(getIndexByDate(row, indx_start))
    ...
    for (var i = 0; i < row.children.length; i++) {
      if (hidden_counter.containsKey(dates[i].toString())) {
        ...
        new CLElement(new AnchorElement())
          ...
          ..addAction((e) => _renderHidden(e, dates[i], h_l), 'mousedown')
  ...
  CLElement _eventDom(
    ...
    final div = new CLElement(new DivElement())
    ...
    div
      ..append(new SpanElement()..text = event.title)
    ...
    void setDragPos(CLElement drg, Rectangle rect, MouseEvent e) =>
        drg.setStyle({
  ...
  void _renderHidden(MouseEvent e, DateTime date, List<Event> events) {
    e
      ..preventDefault()
      ..stopPropagation();
    if (pop != null) pop.close();
    final cont = new CLElement(new DivElement())..setClass('ui-event-popup');
    final title = '${utils.Calendar.getDayString(date.weekday)}, '
        '${date.day} ${utils.Calendar.getMonthString(date.month)} ${date.year}';
    final title_dom = new CLElement(new DivElement())
      ..setClass('title')
      ..setText(title)
      ..append(new CLElement(new AnchorElement())
        ..addAction((e) {
          pop.close();
        }));
  ...
  TableCellElement getIndexByDate(TableRowElement row, DateTime date) {
